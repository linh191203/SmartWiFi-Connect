...
const SSID_VALUE_REGEX =
  /^\s*(?:ssid|wifi(?:\s*name)?|wifi\s*id|id|network\s*name|ten\s*wifi|t[eê]n\s*wifi|t[eê]n\s*m[aạ]ng)\s*[:=-]\s*(.*)$/i;
...
const SSID_LABEL_REGEX =
  /^\s*(?:ssid|wifi(?:\s*name)?|wifi\s*id|id|network\s*name|ten\s*wifi|t[eê]n\s*wifi|t[eê]n\s*m[aạ]ng)\b/i;
...
function stripSsidPrefix(text) {
  return String(text || "")
    .replace(/^(ssid|wifi(?:\s*name)?|wifi\s*id|id|network\s*name|ten\s*wifi|t[eê]n\s*wifi|t[eê]n\s*m[aạ]ng)\s*[:=-]\s*/i, "")
    .trim();
}
