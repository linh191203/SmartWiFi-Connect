...
class WifiOcrProcessorTest {
    ...
    @Test
    fun extract_englishNameAndPasswordLabel_returnsBothFields() {
        val text = """
            Name: CAFE MỘC
            Password: Cf222222
        """.trimIndent()
        val result = processor.extractWifiCredentials(text)
        assertEquals("CAFE MỘC", result.ssid)
        assertEquals("Cf222222", result.password)
    }

    @Test
    fun extract_englishNameLabelOnly_nextLineValue_returnsBothFields() {
        val text = """
            Name:
            CAFE MỘC
            Password:
    ...
    @Test
    fun extract_signboard_twoColumnRow_withPipe_returnsBothFields() {
        val text = """
            wifi
            Name :            Password :
            CAFE MỘC | Cf222222
    ...
    @Test
    fun extract_signboard_twoColumnRow_withWideSpaces_returnsBothFields() {
        val text = """
            Name : Password :
            CAFE MỘC    Cf222222
        """.trimIndent()
    ...
    @Test
    fun extract_inlineNameAndPassword_sameLine_returnsCleanSsidAndPassword() {
        val text = "Name: CAFE MỘC Password: Cf222222"
        val result = processor.extractWifiCredentials(text)
        assertEquals("CAFE MỘC", result.ssid)
        assertEquals("Cf222222", result.password)
    }
    ...
    @Test
    fun extract_wifiAndPassLabels_fromCafeSign_returnsBothFields() {
        val text = """
            WIFI : The Monday Coffee
            PASS : TheMondayCoffee
        """.trimIndent()
        val result = processor.extractWifiCredentials(text)
        assertEquals("The Monday Coffee", result.ssid)
        assertEquals("TheMondayCoffee", result.password)
    }

    @Test
    fun extract_nameAndPasswordLabelOnly_fromPrintedSign_returnsBothFields() {
        val text = """
            Name :
            BINH MINH GROUP
            Password :
        ...
        """.trimIndent()
        val result = processor.extractWifiCredentials(text)
        assertEquals("BINH MINH GROUP", result.ssid)
        assertEquals("Bm222222", result.password)
    }

    @Test
    fun extract_idLine_followedByUnlabeledPassword_returnsBothFields() {
        val text = """
            KINH CHAO QUY KHACH
            ID DONG PHUC LINH TRAN
            linhtran103
        """.trimIndent()
        val result = processor.extractWifiCredentials(text)
    ...
    @Test
    fun recognize_emulatorWifiSigns_logsRawTextAndParsedCredentials_whenSamplesPresent() = runBlocking {
        ...
        try {
            for (sampleName in sampleNames) {
                val sampleUri = context.findSampleImageUri(sampleName)
                assumeTrue(sampleUri != null)

                val bitmap = context.contentResolver.openInputStream(sampleUri!!)?.use { inputStream ->
                ...
                assertNotNull("Unable to decode $sampleUri", bitmap)

                bitmap!!.useBitmap { currentBitmap ->
                    val text = processor.recognizeText(currentBitmap)
                    val credentials = processor.extractWifiCredentials(text)

                    println("OCR_SAMPLE::$sampleName")
                    println(text)
                    println("OCR_PARSED::$sampleName::$credentials")

                    assertTrue("Expected non-blank OCR output for $sampleName", text.isNotBlank())
                }
            }
        } finally {
    ...
    private fun android.content.Context.findSampleImageUri(sampleName: String): android.net.Uri? {
        val projection = arrayOf(MediaStore.Images.Media._ID)
        val selection = "${MediaStore.Images.Media.DISPLAY_NAME} = ? AND ${MediaStore.Images.Media.RELATIVE_PATH} = ?"
        val selectionArgs = arrayOf(sampleName, "DCIM/SmartWiFi/")
