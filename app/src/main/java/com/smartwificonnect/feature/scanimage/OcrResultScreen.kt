...
fun OcrResultScreen(
    ...
    onRefreshNearbyWifi: () -> Unit = {},
    onSelectNearbyNetwork: (String) -> Unit = {},
    onConnectWifi: () -> Unit = {},
) {
    Scaffold(
        ...
        if (state.isLoading) {
            ...
            LazyColumn(
                ...
                item {
                    RecognitionStatusBanner(state = state)
                }
                ...
                item {
                    Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                        Text(
                            text = if (hasCredentials) "Thông tin Wi-Fi" else "Nhập thủ công",
                        ...
                        Text(
                            text = if (hasCredentials) {
                                "Ứng dụng đã điền tên mạng và mật khẩu đọc được. Bạn có thể sửa hoặc chọn SSID trong danh sách Wi-Fi thật trước khi kết nối."
                            } else {
                                "Bạn vẫn có thể nhập thông tin Wi-Fi thủ công bên dưới."
                            },
                ...
                if (state.password.isNotBlank() || state.nearbyWifiStatus.isNotBlank()) {
                    item {
                        NearbyWifiSelectionCard(
                            networks = state.nearbyNetworks,
                            status = state.nearbyWifiStatus,
                            onRefresh = onRefreshNearbyWifi,
                            onSelect = onSelectNearbyNetwork,
                        )
                    }
                }
...
@Composable
private fun NearbyWifiSelectionCard(
                    ...
                    Surface(
                        ...
                        Row(
                            ...
                            Column(
                                ...
                                Text(
                                    text = network.toNearbyMetadataLabel(),
                                    style = MaterialTheme.typography.bodySmall,
                                    color = OcrBody,
                                )
                            }
                            Text(
                                text = "${network.signalLevel.coerceIn(0, 4)}/4",
...
private fun NearbyNetwork.toNearbyMetadataLabel(): String {
    val parts = buildList {
        securityLabel?.takeIf { it.isNotBlank() }?.let(::add)
        frequencyMhz?.takeIf { it > 0 }?.let { mhz ->
            add(
                if (mhz >= 5000) {
                    String.format(java.util.Locale.US, "%.1f GHz", mhz / 1000f)
                } else {
                    "2.4 GHz"
                },
            )
        }
        signalDbm?.let { add("$it dBm") }
        bssid?.let { add("BSSID $it") }
    }
    return if (parts.isEmpty()) "Không có dữ liệu chi tiết" else parts.joinToString(" • ")
}

@Composable
private fun OcrEmptyStateCard() {
    Card(
        shape = RoundedCornerShape(24.dp),
        colors = CardDefaults.cardColors(containerColor = OcrSurface),
    ) {
        Column(
            ...
            Text(
                text = "Không nhận ra thông tin Wi-Fi",
                style = MaterialTheme.typography.titleMedium,
                fontWeight = FontWeight.ExtraBold,
                color = OcrTitle,
            ...
            Text(
                text = "Hãy chụp rõ hơn hoặc dùng ảnh có chứa nhãn SSID/mật khẩu. Bạn cũng có thể tự nhập thủ công bên dưới.",
                style = MaterialTheme.typography.bodySmall,
                color = OcrBody,
                textAlign = TextAlign.Center,
            )
...
@Composable
private fun RecognitionStatusBanner(state: MainUiState) {
    val hasCredentials = state.ssid.isNotBlank() || state.password.isNotBlank()
    val needsReview = hasCredentials && (
        state.sourceFormat == "ocr_local_review" ||
            ((state.confidence ?: 1.0) < 0.72)
    )
    val confidenceSuffix = state.confidence?.let { " (${(it * 100).toInt()}%)" }.orEmpty()

    val icon = when {
        !hasCredentials -> Icons.Outlined.WifiOff
        needsReview -> Icons.Outlined.Warning
        else -> Icons.Outlined.CheckCircle
    }
    val text = when {
        !hasCredentials -> "Chưa đọc được thông tin Wi-Fi"
        needsReview -> "OCR đã điền gợi ý, cần kiểm tra lại$confidenceSuffix"
        else -> "Đã đọc được thông tin Wi-Fi$confidenceSuffix"
    }
    val tint = when {
        !hasCredentials -> Color(0xFFB3261E)
        needsReview -> Color(0xFF9A6700)
        else -> OcrBrand
    }
    val bg = when {
        !hasCredentials -> Color(0xFFFDECEC)
        needsReview -> Color(0xFFFFF4E5)
...
private fun WifiConnectionStatus(state: WifiConnectionState) {
    when (state) {
        ...
        is WifiConnectionState.Connecting -> {
            StatusSurface(
                text = "Đang kết nối tới ${state.ssid} và xác minh Internet...",
        ...
        is WifiConnectionState.Connected -> {
            StatusSurface(
                text = "Kết nối Wi-Fi thành công: ${state.ssid}",
    ...
    SmartWifiAppTheme {
        OcrResultScreen(
            state = MainUiState(
                ...
                statusMessage = "AI đã điền thông tin. Hãy kiểm tra SSID/mật khẩu rồi bấm Kết nối.",
            ),
            onBackClick = {},
            onSsidChange = {},
            onPasswordChange = {},
