package com.smartwificonnect.feature.review
...
import com.smartwificonnect.AiValidationState
import com.smartwificonnect.MainUiState
import com.smartwificonnect.NearbyNetwork
import com.smartwificonnect.SsidSuggestionState
...
@Composable
private fun ReviewNearbyWifiCard(
    ...
    Card(
        ...
        Column(
            ...
            Text(
                text = if (status.isNotBlank()) {
                    status
                } else {
                    "Nếu OCR chỉ đọc được mật khẩu hoặc SSID chưa chắc chắn, hãy chọn đúng mạng trong danh sách scan thật."
                },
                style = MaterialTheme.typography.bodySmall,
                color = ReviewBody,
            )
            if (networks.isEmpty()) {
                OutlinedButton(
                    ...
                    Text("Quét Wi-Fi xung quanh", fontWeight = FontWeight.Bold)
                }
            } else {
                    ...
                    Surface(
                        ...
                        Row(
                            ...
                            Column(modifier = Modifier.weight(1f)) {
                                ...
                                Text(
                                    text = network.toReviewNearbyLabel(),
                                    style = MaterialTheme.typography.bodySmall,
                                    color = ReviewBody,
                                )
                            }
...
private fun NearbyNetwork.toReviewNearbyLabel(): String {
    val parts = buildList {
    ...
    return if (parts.isEmpty()) "Đã sắp xếp theo tín hiệu mạnh đến yếu" else parts.joinToString(" • ")
}
...
private fun ReviewActions(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
        Button(
            ...
            Text(
                text = when {
                    isConnected -> "Kết nối Wi-Fi thành công"
