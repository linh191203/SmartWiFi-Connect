...
class WifiOcrProcessor : WifiOcrEngine {
    ...
    suspend fun recognize(bitmap: Bitmap): WifiOcrRecognitionResult {
        ...
        for ((index, pass) in recognitionPasses.withIndex()) {
            val candidateText = recognizeTextWithPass(bitmap, pass)
            if (candidateText.isNotBlank() && candidateText !in textCandidates) {
                textCandidates += candidateText
            }

            if (index >= minimumPassesBeforeEarlyStop) {
                val interim = WifiOcrResultSelector.selectBestResult(textCandidates)
                if (interim.hasHighConfidenceCompleteCredentials()) {
                    break
                }
            }
        }

        return WifiOcrResultSelector.selectBestResult(textCandidates)
    }
    ...
    companion object {
        ...
        private val recognitionPasses = listOf(
            ...
            RecognitionPass(autoCropPaper = true, variant = RecognitionVariant.AUTO_CONTRAST),
            ...
            RecognitionPass(cropRegion = CropRegion.CENTER, variant = RecognitionVariant.HIGH_CONTRAST),
            ...
            RecognitionPass(cropRegion = CropRegion.BOTTOM_RIGHT, variant = RecognitionVariant.AUTO_CONTRAST),
            RecognitionPass(cropRegion = CropRegion.BOTTOM_RIGHT, variant = RecognitionVariant.SHARPENED_AUTO_CONTRAST),
            RecognitionPass(rotationDegrees = 90f, variant = RecognitionVariant.HIGH_CONTRAST),
            RecognitionPass(rotationDegrees = 270f, variant = RecognitionVariant.HIGH_CONTRAST),
            RecognitionPass(rotationDegrees = -12f, autoCropPaper = true, variant = RecognitionVariant.HIGH_CONTRAST),
            RecognitionPass(rotationDegrees = 12f, autoCropPaper = true, variant = RecognitionVariant.HIGH_CONTRAST),
        )
